...
function calculateCost(route, body) {
    ...
    if (route.startsWith('media/')) return { cost: 0, muapiEndpoint: route };
...
async function firestoreDeductCredits(projectId, docPath, cost, accessToken, attempt = 0) {
    ...
    if (!commitRes.ok) {
        ...
        if (
            attempt < 3 &&
            (commitRes.status === 409 || errBody.includes('ABORTED') || errBody.includes('FAILED_PRECONDITION'))
        ) {
...
function extractMediaTokenFromUrl(value, request) {
...
async function unwrapIncomingMediaUrls(value, env, request) {
    ...
    if (typeof value === 'string') {
        const token = extractMediaTokenFromUrl(value, request);
        if (!token) return value;

        try {
            const payload = await decryptMediaToken(token, env);
            return payload.url || value;
        } catch {
            return value;
        }
    }
