...
export async function onRequest(context) {
  ...
  try {
    ...
    if (!invoiceId) return jsonError('Falta invoiceId', 400);

    const invoiceDoc = await getDoc(env.FIREBASE_PROJECT_ID, `private_invoices/${invoiceId}`, firestoreToken);
    const invoice = invoiceDoc.data;
    if (!invoice?.client?.email) return jsonError('La factura no tiene email de cliente', 400);

    const gmailToken = await getGmailDelegatedToken(env);
    const email = buildInvoiceEmail(env.GMAIL_SENDER, invoice);
    const gmailResult = await sendGmail(gmailToken, email);

    const now = new Date().toISOString();
    await commitWrites(env.FIREBASE_PROJECT_ID, firestoreToken, [{
      update: {
        name: docName(env.FIREBASE_PROJECT_ID, `private_invoices/${invoiceId}`),
        fields: toFields({
          emailSentAt: now,
    ...
    return jsonError(err.message || 'Error enviando factura', 500);
  }
}

function buildInvoiceEmail(sender, invoice) {
  const clientName = escapeHtml(invoice.client.fullName || 'cliente');
  const invoiceNumber = escapeHtml(invoice.invoiceNumber);
  const issuer = invoice.issuer || {};
  const subject = `Factura ${invoice.invoiceNumber} - KreateIA`;
  const taxText = invoice.taxRate === 0
          ...
          <table role="presentation" width="680" cellpadding="0" cellspacing="0" style="width:680px;max-width:100%;background:#ffffff;border-radius:18px;overflow:hidden;border:1px solid #e2e8f0">
            <tr>
              <td style="padding:24px 28px;background:#0f172a;color:#fff">
                ...
                <div style="font-size:13px;color:#cbd5e1;margin-top:6px">Factura y confirmación de servicio</div>
              </td>
            </tr>
            <tr>
              <td style="padding:28px">
                <p style="font-size:16px;line-height:1.6;margin:0 0 18px">Hola ${clientName},</p>
                <p style="font-size:16px;line-height:1.6;margin:0 0 22px">Gracias por confiar en <strong>KreateIA</strong>. Te enviamos el resumen de tu factura <strong>${invoiceNumber}</strong>.</p>
                <table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 18px;border-collapse:collapse">
                  <tr>
                    ...
                    <td style="width:50%;vertical-align:top;padding:12px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px">
                      ...
                      <div style="font-weight:bold">${escapeHtml(invoice.client.fullName || '')}</div>
                      <div style="font-size:13px;color:#64748b">${escapeHtml(invoice.client.taxId || '')}</div>
                      <div style="font-size:13px;color:#64748b">${escapeHtml(invoice.client.address || '')}</div>
                    </td>
                  </tr>
                </table>
                ...
                <p style="font-size:13px;line-height:1.6;color:#64748b;margin:22px 0 0">Si necesitas que modifiquemos algún dato fiscal, responde directamente a este correo.</p>
              </td>
            </tr>
            <tr>
