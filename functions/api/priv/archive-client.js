    ...
    await commitWrites(env.FIREBASE_PROJECT_ID, accessToken, [{
      update: {
        ...
        fields: toFields({
          ...
          archivedBy: archive ? { uid: staff.uid, email: staff.email } : null,
          updatedAt: now,
        }),
      },
      updateMask: { fieldPaths: ['archivedAt', 'archivedBy', 'updatedAt'] },
    }]);

    return json({ ok: true });
  } catch (err) {
    return jsonError(err.message || 'Error archivando cliente', 500);
  }
}
...
async function getDoc(projectId, path, accessToken, allowMissing = false) {
  ...
  return { exists: true, data: fromFields(raw.fields || {}) };
}
