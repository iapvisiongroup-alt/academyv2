...
export async function onRequest(context) {
  ...
  try {
    ...
    if (!leadId) return jsonError('leadId invalido', 400);

    const update = normalizeLeadUpdate(body || {}, staff);
    const now = new Date().toISOString();
    const fieldPaths = Object.keys(update);

    await commitWrites(env.FIREBASE_PROJECT_ID, accessToken, [{
      update: {
        name: docName(env.FIREBASE_PROJECT_ID, `private_leads/${leadId}`),
        fields: toFields({
          ...update,
          updatedAt: now,
    ...
    return json({ ok: true, leadId, ...update, updatedAt: now });
  } catch (err) {
    return jsonError(err.message || 'Error actualizando lead', 500);
  }
}

function normalizeLeadUpdate(body, staff) {
  const update = {};
  const status = String(body.status || '').trim();

  if (status) {
    if (!VALID_STATUSES.has(status)) throw new Error('Estado de lead no valido');
    update.status = status;
    if (status !== 'nuevo') {
      update.lastContactAt = new Date().toISOString();
      update.lastContactBy = { uid: staff.uid, email: staff.email };
    }
  }

  if ('notes' in body) {
    update.notes = String(body.notes || '').trim().slice(0, 4000);
  }

  if ('contactChannel' in body) {
    update.contactChannel = String(body.contactChannel || '').trim().slice(0, 80);
  }

  if (!Object.keys(update).length) throw new Error('No hay cambios para guardar');
  return update;
}
