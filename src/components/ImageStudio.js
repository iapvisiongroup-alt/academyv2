...
const getApiId = (uiId, mode) => {
    ...
    if (uiId === 'kreate-2') {
        ...
        if (mode === 'v2v') return 'sd-2-omni-reference';
        return 'sd-2-t2v';
    }
...
const getVideoCost = (apiId, duration, quality) => {
    ...
    if (apiId === 'sd-2-t2v') {
        costPerSecond = q === 'high' ? 3.0 : 2.4;
    }

    if (apiId === 'sd-2-i2v-480p') {
        costPerSecond = q === 'high' ? 3.0 : 2.4;
    }

    if (apiId === 'sd-2-omni-reference') {
        costPerSecond = q === 'high' ? 6.0 : 4.2;
    }

    if (apiId === 'sd-2-extend') {
        costPerSecond = q === 'high' ? 3.0 : 2.4;
    }
...
const buildVideoParams = ({
    ...
    if (finalApiId === 'sd-2-omni-reference') {
        let finalPrompt = promptText || 'Create a cinematic video using the reference video';

        if (!finalPrompt.includes('@video1')) {
            finalPrompt = `@video1 ${finalPrompt}`;
        }

        return {
            prompt: finalPrompt,
            images_list: [],
            video_files: [uploadedVideoUrl],
            audio_files: [],
            aspect_ratio: selectedAr,
            duration,
            quality
    ...
    if (finalApiId === 'sd-2-extend') {
        if (!lastGenerationId) {
            throw new Error('No hay request_id válido para extender este vídeo.');
        }

        return {
            request_id: lastGenerationId,
            prompt: promptText || 'Continue the video seamlessly',
            images_list: [],
            video_files: [],
            audio_files: [],
            aspect_ratio: selectedAr,
...
export function VideoStudio() {
    ...
    container.className =
        'w-full h-full flex flex-col items-center bg-[#050505] relative p-2 md:p-6 pb-24 overflow-y-auto custom-scrollbar overflow-x-hidden';
    ...
    generateBtn.className =
        'bg-[#FFB000] text-black px-6 md:px-8 py-3 md:py-3.5 rounded-xl md:rounded-[1.5rem] font-black text-sm md:text-base hover:shadow-[0_0_20px_rgba(255,176,0,0.4)] active:scale-95 transition-all flex items-center justify-center gap-1.5 md:gap-2.5 w-full sm:w-auto shadow-lg shrink-0 mt-2 sm:mt-0';
    ...
    hero.className =
        'flex flex-col items-center mb-6 md:mb-16 mt-4 md:mt-0 animate-fade-in-up transition-all duration-700 shrink-0';
    hero.innerHTML = `
    ...
    promptWrapper.className =
        'w-full max-w-4xl relative z-40 animate-fade-in-up shrink-0 px-2 md:px-0';

    const bar = document.createElement('div');
    bar.className =
        'w-full bg-[#111]/90 backdrop-blur-xl border border-white/10 rounded-[1.5rem] md:rounded-[2.5rem] p-3 md:p-5 flex flex-col gap-3 md:gap-5 shadow-3xl';
    ...
    textarea.className =
        'flex-1 bg-transparent border-none text-white text-sm md:text-xl placeholder:text-muted focus:outline-none resize-none pt-2 md:pt-2.5 leading-relaxed min-h-[40px] max-h-[150px] md:max-h-[250px] overflow-y-auto custom-scrollbar';
    textarea.rows = 1;

    textarea.oninput = () => {
        textarea.style.height = 'auto';
        textarea.style.height =
            Math.min(textarea.scrollHeight, window.innerWidth < 768 ? 120 : 250) + 'px';
    };
    ...
    videoPickerBtn.className =
        'w-10 h-10 md:w-12 md:h-12 shrink-0 rounded-xl md:rounded-2xl border transition-all flex items-center justify-center relative overflow-hidden mt-1 md:mt-1.5 bg-white/5 border-white/10 hover:bg-white/10 hover:border-[#FFB000]/40 group';
    ...
    videoIconEl.innerHTML = `
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-white/40 group-hover:text-[#FFB000] transition-colors">
    ...
    videoReadyEl.innerHTML = `
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-[#FFB000]">
    ...
    showVideoIcon = () => {
        videoIconEl.classList.remove('hidden');
        videoIconEl.classList.add('flex');

        videoSpinnerEl.classList.add('hidden');
        videoSpinnerEl.classList.remove('flex');

        videoReadyEl.classList.add('hidden');
        videoReadyEl.classList.remove('flex');
    ...
    const showVideoSpinner = () => {
        videoIconEl.classList.add('hidden');
        videoIconEl.classList.remove('flex');

        videoSpinnerEl.classList.remove('hidden');
        videoSpinnerEl.classList.add('flex');
    ...
    const showVideoReady = () => {
        ...
        videoReadyEl.classList.remove('hidden');
        videoReadyEl.classList.add('flex');
    ...
    videoFileInput.onchange = async (e) => {
        ...
        try {
            if (typeof muapi.uploadFile !== 'function') {
                throw new Error('Función de subida no detectada en la librería.');
            }
    ...
    extendBanner.className =
        'hidden items-center justify-between gap-2 px-4 py-2.5 mx-2 mt-2 bg-[#3B82F6]/10 border border-[#3B82F6]/20 rounded-xl text-xs text-[#3B82F6]';
    ...
    bottomRow.className =
        'flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 px-1 md:px-2 pt-3 border-t border-white/5';

    const controlsLeft = document.createElement('div');
    controlsLeft.className =
        'flex flex-wrap items-center justify-center sm:justify-start gap-1.5 md:gap-2.5 w-full sm:w-auto';

    const createControlBtn = (icon, label, id) => {
        const btn = document.createElement('button');
        btn.id = id;
        btn.className =
            'flex items-center gap-1.5 md:gap-2.5 px-2.5 py-1.5 md:px-4 md:py-2.5 bg-white/5 hover:bg-white/10 rounded-xl md:rounded-2xl transition-all border border-white/5 group whitespace-nowrap flex-1 sm:flex-none justify-center';

        btn.innerHTML = `
            ${icon}
            <span id="${id}-label" class="text-[10px] md:text-xs font-bold text-white group-hover:text-[#FFB000] truncate max-w-[80px] md:max-w-none">${label}</span>
            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="4" class="opacity-20 group-hover:opacity-100 transition-opacity shrink-0">
                <path d="M6 9l6 6 6-6"/>
            </svg>
        `;
    ...
    const modelBtn = createControlBtn(
        `<div class="w-4 h-4 md:w-5 md:h-5 bg-[#FFB000] rounded flex items-center justify-center shadow-[0_0_10px_rgba(255,176,0,0.3)] shrink-0"><span class="text-[8px] md:text-[10px] font-black text-black">K</span></div>`,
        selectedModelName,
        'v-model-btn'
    );
    ...
    const durationBtn = createControlBtn(
        `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="opacity-60 text-white/50 shrink-0 md:w-4 md:h-4"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>`,
        `${selectedDuration}s`,
        'v-duration-btn'
    );

    const qualityBtn = createControlBtn(
        `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="opacity-60 text-white/50 shrink-0 md:w-4 md:h-4"><path d="M12 2l3 7h7l-5.5 4.5L18.5 21 12 16.8 5.5 21l2-7.5L2 9h7z"/></svg>`,
        'Básica',
        'v-quality-btn'
    );
    ...
    inlineInstructions.className =
        'w-full text-center text-white/30 text-sm flex flex-col items-center gap-1 md:gap-2 py-2 px-4';
    inlineInstructions.innerHTML = `
        <p class="text-[10px] md:text-xs text-white/20">Puedes lanzar varios vídeos a la vez sin esperar.</p>
    `;
    ...
    function updateControlsForModel() {
        ...
        generateBtn.innerHTML = `
            Generar ✨ 
            <span class="bg-black/20 px-2 py-0.5 rounded-md text-[10px] md:text-xs font-mono ml-1 shadow-inner border border-black/10">${cost} 🪙</span>
        `;
    }
    ...
    dropdown.className =
        'fixed z-[999999] transition-all opacity-0 pointer-events-none scale-95 glass rounded-2xl md:rounded-3xl p-2 md:p-3 shadow-2xl border border-white/10 flex flex-col bg-[#111]/95 backdrop-blur-xl';
    ...
    const openDropdown = (type, anchorBtn) => {
        ...
        if (type === 'model') {
            dropdown.innerHTML = `
                <div class="text-[10px] font-bold text-white/50 uppercase tracking-widest px-2 py-2 border-b border-white/5 mb-2">
                    Modelos KreateIA
                </div>
            ...
            V_MODELS.forEach((m) => {
                ...
                item.className =
                    'flex items-center justify-between p-3 hover:bg-white/5 rounded-xl cursor-pointer transition-all';

                item.innerHTML = `
                    <span class="text-xs md:text-sm font-bold text-white">${m.name}</span>
                    ${
                        selectedUiId === m.uiId
                            ? '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#FFB000" stroke-width="4"><polyline points="20 6 9 17 4 12"/></svg>'
                            : ''
                    }
                `;
        ...
        if (type === 'ar') {
            ...
            if (finalApiId === 'sd-2-omni-reference' || finalApiId === 'sd-2-extend') {
            ...
            ars.forEach((ar) => {
                ...
                item.innerHTML = `
                    <span class="text-xs md:text-sm font-bold text-white">${ar}</span>
                    ${
                        selectedAr === ar
                            ? '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#FFB000" stroke-width="4"><polyline points="20 6 9 17 4 12"/></svg>'
                            : ''
        ...
        if (type === 'duration') {
            ...
            durations.forEach((d) => {
                ...
                item.innerHTML = `
                    <span class="text-xs md:text-sm font-bold text-white">${d}s</span>
                    ${
                        selectedDuration === d
                            ? '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#FFB000" stroke-width="4"><polyline points="20 6 9 17 4 12"/></svg>'
                            : ''
        ...
        if (type === 'quality') {
            ...
            qualities.forEach((q) => {
                ...
                item.innerHTML = `
                    <span class="text-xs md:text-sm font-bold text-white">${q.name}</span>
                    ${
    ...
    galleryWrapper.className =
        'w-full max-w-6xl mt-4 md:mt-8 flex-1 flex flex-col shrink-0 px-2 md:px-0';

    const galleryHeader = document.createElement('h3');
    galleryHeader.className =
        'text-[10px] md:text-xs font-bold text-white/40 uppercase tracking-widest mb-3 md:mb-4 px-2 hidden';
    galleryHeader.textContent = 'Tus Creaciones';

    const galleryGrid = document.createElement('div');
    galleryGrid.className =
        'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-5 w-full';
    ...
    const renderCard = (entry, isPrepend = false) => {
        ...
        card.className =
            'relative aspect-video rounded-xl md:rounded-2xl overflow-hidden bg-white/5 border border-white/10 group animate-fade-in-up';
        ...
        card.innerHTML = `
            ...
            <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-3 md:p-4">
                <p class="text-white text-[10px] md:text-xs font-medium line-clamp-2 md:line-clamp-3 mb-2 md:mb-3 shadow-black drop-shadow-md leading-tight">
                    ${entry.prompt || 'Vídeo generado'}
                </p>

                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-1.5">
                        <span class="text-[8px] md:text-[10px] text-white/70 bg-black/60 px-1.5 py-0.5 md:px-2 md:py-1 rounded-md backdrop-blur-sm border border-white/10">
                            ${entry.duration || 5}s
                        </span>

                        <span class="text-[8px] md:text-[10px] text-[#FFB000] font-bold bg-[#FFB000]/10 px-1.5 py-0.5 md:px-2 md:py-1 rounded-md backdrop-blur-sm border border-[#FFB000]/20">
                            ${entry.quality === 'high' ? 'Alta' : 'Básica'}
                        </span>
                    </div>

                    <div class="flex items-center gap-2">
                        <button class="extend-btn p-1.5 md:p-2 bg-[#3B82F6]/20 hover:bg-[#3B82F6] text-[#3B82F6] hover:text-white rounded-lg md:rounded-xl backdrop-blur-md transition-all border border-[#3B82F6]/30" title="Extender vídeo">
    ...
    const loadFirebaseHistory = async (user) => {
        try {
            ...
            console.error('Error cargando historial de vídeo:', error);
        }
    };
    ...
    generateBtn.onclick = async () => {
        ...
        try {
            ...
            if (finalApiId === 'sd-2-t2v' && !promptText) {
                return alert('Escribe un prompt para generar el vídeo.');
            }

            if (finalApiId === 'sd-2-i2v-480p' && !uploadedImageUrl) {
                return alert('Sube una imagen de referencia.');
            }

            if (finalApiId === 'sd-2-omni-reference' && !uploadedVideoUrl) {
                return alert('Sube un vídeo de referencia.');
            }

            if (finalApiId === 'sd-2-extend' && !lastGenerationId) {
                return alert('No hay request_id válido para extender este vídeo.');
            }
            ...
            try {
                ...
                console.error('Error comprobando saldo:', err);
            ...
            loadingCard.className =
                'relative aspect-video rounded-xl md:rounded-2xl overflow-hidden bg-white/5 border border-white/10 flex flex-col items-center justify-center animate-fade-in-up';
            ...
            loadingCard.innerHTML = `
                ...
                <div class="z-10 flex flex-col items-center gap-2 md:gap-3">
                    ...
                    <span class="status-text text-xs md:text-sm font-bold text-[#FFB000] animate-pulse">
                        Conectando motor...
                    </span>
                </div>

                <div class="absolute bottom-2 md:bottom-4 left-2 right-2 md:left-4 md:right-4 text-[8px] md:text-[10px] text-center text-white/40 line-clamp-2 px-1 md:px-2 leading-tight">
                    ${promptText || 'Procesando vídeo'}
                </div>
            `;
            ...
            if (!req.ok) {
                throw new Error(res?.error || res?.message || 'Error lanzando generación de vídeo.');
            }

            const rid = res?.request_id || res?.output?.id || res?.id;
            ...
            if (rid && !finalUrl) {
                loadingCard.querySelector('.status-text').textContent = 'Renderizando vídeo...';
                ...
                while (attempts < 150) {
                    ...
                    const poll = await fetch(`/api/v1/predictions/${rid}/result`, {
                        headers: {
                            Authorization: `Bearer ${token}`
                        }
                    });
                    ...
                    if (poll.ok) {
                        ...
                        if (
                            ...
                            pollRes?.output?.status === 'failed'
                        ) {
                            throw new Error(
                                pollRes?.error ||
                                    pollRes?.output?.error ||
                                    'El vídeo falló en MuAPI.'
                            );
                        }
                    }
            ...
            if (!finalUrl) {
                throw new Error('El motor no devolvió URL final.');
            }
            ...
            try {
                ...
                console.error('No se pudieron descontar créditos:', billingErr);
            }

            const cleanPrompt = String(params.prompt || promptText || '')
                .replace('@image1', '')
                .replace('@video1', '')
                .trim();
            ...
            try {
                ...
                console.error('No se pudo guardar en Firebase:', saveErr);
            }
            ...
            if (loadingCard) {
                loadingCard.innerHTML = `
                    ...
                    <div class="z-10 flex flex-col items-center gap-1 md:gap-2 p-2 md:p-4 text-center">
                        ...
                        <span class="text-[8px] md:text-[10px] font-bold text-red-400">
                            Error interno
                        </span>
                        <span class="text-white/50 text-[6px] px-2 break-words w-full">
                            ${String(errorFatal.message || '').slice(0, 100)}
                        </span>
                        <button class="retry-btn mt-1 bg-white/10 px-2 py-1 rounded-md text-[8px] text-white hover:bg-white/20 border border-white/10">
                            Cerrar
                        </button>
                    </div>
                `;
                ...
                if (retryBtn) {
                    retryBtn.onclick = () => loadingCard.remove();
                }
            }
        } finally {
            setTimeout(() => {
