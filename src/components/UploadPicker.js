...
export function createUploadPicker({ anchorContainer, onSelect, onClear, maxImages: initialMaxImages = 1 }) {
    ...
    trigger.className = 'w-10 h-10 shrink-0 rounded-xl border transition-all flex items-center justify-center relative overflow-hidden mt-1.5 bg-white/5 border-white/10 hover:bg-white/10 hover:border-[#3B82F6]/40 group';

    // State: icon
    const iconState = document.createElement('div');
    iconState.className = 'flex items-center justify-center w-full h-full';
    iconState.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-white/50 group-hover:text-[#3B82F6] transition-colors"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>`;

    // State: spinner
    const spinnerState = document.createElement('div');
    spinnerState.className = 'hidden items-center justify-center w-full h-full';
    spinnerState.innerHTML = `<span class="animate-spin text-[#3B82F6] text-sm">◌</span>`;
    ...
    countBadge.className = 'absolute bottom-0.5 right-0.5 min-w-[16px] h-4 bg-[#3B82F6] rounded-full flex items-center justify-center px-0.5';
    ...
    const showIcon = () => {
        ...
        trigger.classList.remove('border-[#3B82F6]/60');
    ...
    const updateTrigger = () => {
        ...
        trigger.classList.add('border-[#3B82F6]/60');
        ...
        if (count > 1) {
            ...
            countBadge.className = 'absolute bottom-0.5 right-0.5 min-w-[16px] h-4 bg-white/80 rounded-full flex items-center justify-center px-0.5 border border-[#3B82F6]/60';
    ...
    // CAMBIO IMPORTANTE AQUÍ TAMBIÉN: Posición fixed para evitar cortes
    panel.className = 'fixed z-[999999] opacity-0 pointer-events-none scale-95 origin-bottom-left glass rounded-3xl p-3 shadow-2xl border border-white/10 w-72 transition-all bg-[#111]/95 backdrop-blur-xl';
    ...
    const renderPanel = () => {
        ...
        headerLeft.innerHTML = `<span class="text-[10px] font-bold text-white/50 uppercase tracking-widest">Reference Images</span>`;
        if (isMulti) {
            const hint = document.createElement('span');
            hint.className = 'text-[9px] text-white/40';
        ...
        if (isMulti && selectedEntries.length > 0) {
            ...
            doneBtn.type = 'button';
            doneBtn.className = 'flex items-center gap-1 px-3 py-1.5 bg-[#3B82F6] text-white rounded-xl text-xs font-black transition-all hover:scale-105';
        ...
        uploadNewBtn.className = 'flex items-center gap-1.5 px-3 py-1.5 bg-[#3B82F6]/10 hover:bg-[#3B82F6]/20 text-[#3B82F6] rounded-xl text-xs font-bold transition-all border border-[#3B82F6]/20';
        ...
        if (history.length === 0) {
            ...
            empty.innerHTML = `
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="text-white/50"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                <span class="text-xs text-white/50">No uploads yet</span>
            `;
        ...
        history.forEach(entry => {
            ...
            cell.className = `relative rounded-xl overflow-hidden border-2 cursor-pointer group/cell aspect-square transition-all ${isSelected ? 'border-[#3B82F6] shadow-glow' : 'border-white/10 hover:border-white/30'}`;
            ...
            if (isSelected) {
                ...
                badge.className = 'absolute top-1 left-1 min-w-[20px] h-5 bg-[#3B82F6] rounded-full flex items-center justify-center px-1';
                if (isMulti) {
                    badge.innerHTML = `<span class="text-[10px] font-black text-white">${selIdx + 1}</span>`;
                } else {
                    badge.innerHTML = `<svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="4"><polyline points="20 6 9 17 4 12"/></svg>`;
                }
        ...
        if (isMulti && selectedEntries.length > 0) {
            ...
            bottomBar.innerHTML = `<span class="text-xs text-white/50">${selectedEntries.length} of ${maxImages} selected</span>`;
            const doneBtn2 = document.createElement('button');
            doneBtn2.type = 'button';
            doneBtn2.className = 'px-4 py-1.5 bg-[#3B82F6] text-white rounded-xl text-xs font-black transition-all hover:scale-105';
    ...
    fileInput.onchange = async (e) => {
        ...
        // ELIMINADA LA COMPROBACIÓN DEL API KEY LOCAL
        showSpinner();
        ...
        try {
            ...
            updateTrigger();
            alert(`Fallo al subir la imagen: ${err.message}`);
        }
