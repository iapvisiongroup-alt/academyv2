...
function seekTimelineThumbnail(video){
  const seek=()=>{const clipId=video.closest('[data-clip]')?.dataset.clip,clip=state.clips.find(item=>item.id===clipId);if(clip)video.currentTime=clamp(clip.start,0,Math.max(0,(video.duration||clip.end)-.05))};
