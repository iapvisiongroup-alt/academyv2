...
function seekTimelinePreview(time){const clips=[...state.clips].sort((a,b)=>timelineStartOf(b)-timelineStartOf(a)),clip=clips.find(item=>time>=timelineStartOf(item)&&time<=timelineStartOf(item)+durationOf(item));if(!clip)return;const local=clamp(time-timelineStartOf(clip),0,durationOf(clip));if(state.selectedId!==clip.id)selectClip(clip.id);const asset=assetFor(clip.assetId);if(asset?.type==='video'){els.video.currentTime=clip.start+local}else if(asset?.type==='image'){showSelectedPreview()}state.playhead=time;$('current-time').textContent=fmt(time);updatePlayhead()}
function startPlayheadDrag(event){if(event.button!==0)return;event.preventDefault();event.stopPropagation();document.body.classList.add('playhead-dragging');const move=pointer=>setTimelinePlayhead(timelineTimeFromPointer(pointer.clientX));const end=pointer=>{move(pointer);window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',end);window.removeEventListener('pointercancel',end);document.body.classList.remove('playhead-dragging');seekTimelinePreview(state.playhead)};move(event);window.addEventListener('pointermove',move);window.addEventListener('pointerup',end,{once:true});window.addEventListener('pointercancel',end,{once:true})}
