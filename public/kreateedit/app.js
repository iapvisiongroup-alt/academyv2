...
async function deleteMediaAsset(assetId){const asset=assetFor(assetId);if(!asset||!state.user)return;const used=state.clips.some(c=>c.assetId===assetId)||state.audioTracks.some(c=>c.assetId===assetId);const message=`¿Eliminar “${asset.name}” de KreateEdit?${used?' También se quitará de la línea de tiempo y de tus proyectos KreateEdit.':''}\n\nLas generaciones, fotos y vídeos del SaaS principal no se modificarán.`;if(!confirm(message))return;try{const response=await fetch(`${API_BASE}/media?id=${encodeURIComponent(assetId)}`,{method:'DELETE',headers:await authHeaders()});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||'No se pudo eliminar el archivo.');state.assets=state.assets.filter(a=>a.id!==assetId);state.clips=state.clips.filter(c=>c.assetId!==assetId);state.audioTracks=state.audioTracks.filter(c=>c.assetId!==assetId);state.audioBufferPromises.delete(assetId);if(!selectedClip())state.selectedId=state.clips[0]?.id||state.audioTracks[0]?.id||null;renderAll();markSaved();toast('Archivo eliminado solo de KreateEdit')}catch(error){toast(error.message||'No se pudo eliminar el archivo.')}}
...
function clipHtml(c,index,isAudio=false){const a=assetFor(c.assetId),width=Math.max(42,durationOf(c)*state.pixelsPerSecond),left=timelineStartOf(c)*state.pixelsPerSecond,bg=a?.type==='image'?a.url:'',videoThumb=a?.type==='video'?`<video class="clip-fill clip-video-thumb" src="${a.url}" muted preload="metadata" playsinline></video>`:`<div class="clip-fill" ${bg?`style="background-image:url('${bg}')"`:''}></div>`,volume=Math.round(volumeOf(c)*100);return `<article class="timeline-clip ${isAudio?'audio':'video'} ${c.id===state.selectedId?'selected':''}" data-clip="${c.id}" style="left:${left}px;width:${width}px"><button class="trim-handle trim-left" data-trim-handle="start" data-id="${c.id}" title="Recortar inicio" aria-label="Recortar inicio"></button>${videoThumb}${isAudio?`<canvas class="clip-waveform" data-waveform="${c.id}" aria-hidden="true"></canvas>`:''}<div class="clip-info"><strong>${escapeHtml(c.name)}</strong><span data-clip-duration>${fmt(durationOf(c))}</span></div><div class="clip-flags">${c.muted?'<i data-lucide="volume-x"></i>':''}${fadeInOf(c)||fadeOutOf(c)?'<i data-lucide="audio-waveform"></i>':''}${c.extracted?'<i data-lucide="audio-lines"></i>':''}</div><input class="clip-volume" data-volume-id="${c.id}" type="range" min="0" max="100" value="${volume}" title="Volumen ${volume}%" aria-label="Volumen del clip" /><button class="trim-handle trim-right" data-trim-handle="end" data-id="${c.id}" title="Recortar final" aria-label="Recortar final"></button></article>`}
