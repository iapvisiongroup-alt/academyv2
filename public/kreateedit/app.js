...
const API_BASE = /(^|\.)kreateia\.com$/i.test(location.hostname)
  ? 'https://kreateedit-preview.pages.dev/api'
  : '/api';
const state = {
...
const els = {
  ...
  image:$('preview-image'), empty:$('empty-preview'), videoClips:$('video-clips'), audioClips:$('audio-clips'),
...
function totalDuration(){return state.clips.reduce((sum,c)=>sum+durationOf(c),0)}
...
function snapshot(){return JSON.stringify({clips:state.clips,audioTracks:state.audioTracks,selectedId:state.selectedId,ratio:state.ratio})}
...
function restore(raw){const d=JSON.parse(raw);state.clips=d.clips||[];state.audioTracks=d.audioTracks||[];state.selectedId=d.selectedId||null;state.ratio=d.ratio||'16:9';applyRatio();renderAll()}
...
function projectPayload(){return {project:{name:$('project-name').value,clips:state.clips,audioTracks:state.audioTracks,ratio:state.ratio,updatedAt:new Date().toISOString()}}}
...
async function saveProject(){
  ...
  try{const response=await fetch(`${API_BASE}/project`,{method:'PUT',headers:await authHeaders({'Content-Type':'application/json'}),body:JSON.stringify(projectPayload())});if(!response.ok)throw new Error();$('save-state').textContent='Guardado en Cloudflare'}catch{$('save-state').textContent='Pendiente de guardar'}
}
...
async function addFiles(files){
  ...
  label.textContent='Subiendo a Cloudflare...';
  try{
    for(const file of valid){
      const draft={name:file.name,type:file.type.split('/')[0],url:URL.createObjectURL(file),duration:0};
      try{
        await readMetadata(draft);
        const response=await fetch(`${API_BASE}/upload`,{method:'POST',headers:await authHeaders({'Content-Type':file.type,'X-File-Name':encodeURIComponent(file.name),'X-Media-Duration':String(draft.duration)}),body:file});
...
function renderTimeline(){els.videoClips.innerHTML=state.clips.map((c,i)=>clipHtml(c,i)).join('');els.audioClips.innerHTML=state.audioTracks.map((c,i)=>clipHtml(c,i,true)).join('');document.querySelectorAll('[data-clip]').forEach(el=>el.onclick=()=>selectClip(el.dataset.clip));document.querySelectorAll('[data-move]').forEach(b=>b.onclick=e=>{e.stopPropagation();moveClip(b.dataset.id,Number(b.dataset.move))});const total=totalDuration();$('timeline-duration').textContent=fmt(total);$('total-time').textContent=fmt(total);renderRuler(total);updatePlayhead();refreshIcons()}
function renderRuler(total){const width=Math.max(900,total*state.pixelsPerSecond+100);els.timeRuler.style.width=`${width}px`;els.videoClips.style.width=`${width}px`;els.audioClips.style.width=`${width}px`;let labels='';for(let s=0;s<=Math.max(total,12);s+=2)labels+=`<span class="ruler-label" style="left:${s*state.pixelsPerSecond}px">${fmt(s).slice(0,5)}</span>`;els.timeRuler.innerHTML=labels}
...
function moveClip(id,delta){const i=state.clips.findIndex(c=>c.id===id),to=i+delta;if(i<0||to<0||to>=state.clips.length)return;checkpoint();[state.clips[i],state.clips[to]]=[state.clips[to],state.clips[i]];renderAll()}
...
async function loadCloudWorkspace(){
  ...
  $('save-state').textContent='Cargando nube...';
  try{
    const headers=await authHeaders();
    const [libraryResponse,projectResponse]=await Promise.all([fetch(`${API_BASE}/library`,{headers}),fetch(`${API_BASE}/project`,{headers})]);
    ...
    if(project){
      $('project-name').value=project.name||'Mi proyecto de vídeo';state.clips=project.clips||[];state.audioTracks=project.audioTracks||[];state.ratio=project.ratio||'16:9';
      const ids=new Set(state.assets.map(asset=>asset.id));state.clips=state.clips.filter(clip=>ids.has(clip.assetId));state.audioTracks=state.audioTracks.filter(track=>ids.has(track.assetId));
    }
    state.cloudReady=true;$('save-state').textContent='Guardado en Cloudflare';renderAll();await importFromKreateIA();
  }catch(error){$('save-state').textContent='Error de conexión';toast(error.message||'No se pudo cargar Cloudflare.')}
}
...
onAuthStateChanged(auth,user=>{
  ...
  refreshIcons();if(activeUser)loadCloudWorkspace();else{state.assets=[];state.clips=[];state.audioTracks=[];state.selectedId=null;state.cloudReady=false;$('save-state').textContent='Sesión local';renderAll()}
});
